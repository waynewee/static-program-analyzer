...
bool PQLOptimization::operator()(const STRING_LIST& lhs, const STRING_LIST& rhs) {
    INTEGER lhs_priority = ranks_of_functions.at(lhs[0]) + occurrence_count_.at(lhs[1]) + occurrence_count_.at(lhs[2]);
    INTEGER rhs_priority = ranks_of_functions.at(rhs[0]) + occurrence_count_.at(rhs[1]) + occurrence_count_.at(rhs[2]);
