...
bool ExtractParentRecursive(RelationManager* manager, TNode* root, vector<STMT_IDX> parents) {
    vector<TNode*> children = root->GetChildrenVector();
    if (children.empty()) {
        return true;
    }
...
vector<VAR_NAME*> ExtractVarsFromExpr(TNode* expr) {
    vector<TNode*> children = expr->GetChildrenVector();
    vector<VAR_NAME*> result;
    ...
    for (TNode* child : children) {
        if (child->GetNodeType() == TNode::varName) {
            ...
            vector<VAR_NAME*> list = ExtractVarsFromExpr(child);
            for (VAR_NAME* v : list) {
                result.push_back(v);
            }
        }
...
list<PROC_NAME*> ExtractUsesRecursive(RelationManager* manager, TNode* root, vector<STMT_IDX> parents, PROC_NAME* proc) {
    vector<TNode*> children = root->GetChildrenVector();
    list<PROC_NAME*> lst;
    ...
    // assignment
        ...
        vector<VAR_NAME*> vars = ExtractVarsFromExpr(expr);
